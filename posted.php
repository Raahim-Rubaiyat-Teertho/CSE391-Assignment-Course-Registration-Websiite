    ...
    <?php 
        ...
        if($_SERVER ["REQUEST_METHOD"] == "POST") {
            ...
            if(!empty($name) && !empty($fname) && !empty($sid) && !empty($email) && !empty($slot)) {
                ...
                // show_seats($conn, $slot);

                if(show_seats($conn, $slot) == 0) {
                    // header("Location: change_section.php/");
                    die("<script>alert('This section does not have anymore seats. Please choose another section.');
